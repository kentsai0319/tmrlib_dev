...
void Driver::set_pvt_traj_exec_flag(bool run)
{
	_is_keep_exec_traj = run;
}
bool Driver::get_pvt_traj_exec_flag()
{
	return _is_keep_exec_traj;
}
...
bool Driver::fake_run_pvt_traj(const PvtTraj &pvts)
{
	...
	while (_is_keep_exec_traj) {
		...
		time_now = std::chrono::steady_clock::now();
		point.time = 1.5 * std::chrono::duration_cast<std::chrono::duration<double>>(time_now - time_start).count();
