#ifdef NO_INCLUDE_DIR
#include "tmr_communication.h"
#include "tmr_logger.h"
#else
#include "tmr/tmr_communication.h"
#include "tmr/tmr_logger.h"
#endif

#ifdef _WIN32
// windows socket
#pragma comment (lib, "Ws2_32.lib")
#include <winsock2.h>
#include <ws2tcpip.h>
#else
#include <string.h>
#include <unistd.h>
#include <fcntl.h>
#include <errno.h>
#include <sys/types.h>
#include <sys/socket.h>
#include <sys/select.h>
#include <arpa/inet.h>
#include <netinet/in.h>
#include <netinet/tcp.h>
#endif

namespace tmr
{

//
// SBuffer
//

class SBuffer
{
private:
	std::vector<char> _bytes;

public:
	SBuffer()
	{
		tmr_DEBUG_STREAM("tmr::SBuffer::SBuffer");
	}
	~SBuffer()
	{
		tmr_DEBUG_STREAM("tmr::SBuffer::~SBuffer");
	}

	int length() const
	{
		return _bytes.size();
	}
	char *data()
	{
		return _bytes.data();
	}
	int append(const char *bdata, int blen)
	{
		if (blen <= 0) return 0;

		size_t old_size = _bytes.size();
		size_t new_size = old_size + blen;
		_bytes.resize(new_size);
		for (size_t i = 0; i < size_t(blen); ++i) {
			_bytes[old_size + i] = bdata[i];
		}
		//tmr_DEBUG_STREAM("SBuffer::append " << (int)(blen) << " bytes");
		return blen;
	}
	void pop_front(int len = 1)
	{
		// commit extract
		if (len <= 0) return;

		if (size_t(len) < _bytes.size()) {
			std::vector<char> tmp{ _bytes.begin() + len, _bytes.end() };
			_bytes.clear();
			_bytes.insert(_bytes.end(), tmp.begin(), tmp.end());
		}
		else {
			len = int(_bytes.size());
			_bytes.clear();
		}
		//tmr_DEBUG_STREAM("SBuffer::pop_front ", << (int)(len) << " bytes");
	}
	void clear()
	{
		_bytes.clear();
	}
};

//
// CommRecv
//

class CommRecv
{
private:
	SBuffer _sbuf;
	char *_recv_buf = NULL;
	int _recv_buf_len = 0;
	int _sockfd = -1;
	fd_set _masterfs;
	fd_set _readfs;
	int _rn = 0;
	CommRC _rc = CommRC::OK;

public:
	explicit CommRecv(int recv_buf_len)
	{
		tmr_DEBUG_STREAM("tmr::CommRecv::CommRecv");

		if (recv_buf_len < 512) recv_buf_len = 512;

		_recv_buf = new char[recv_buf_len];
		_recv_buf_len = recv_buf_len;

		memset(_recv_buf, 0, _recv_buf_len);
	}
	~CommRecv()
	{
		tmr_DEBUG_STREAM("tmr::CommRecv::~CommRecv");
		delete _recv_buf;
	}

	bool setup(int sockfd);

	CommRC spin_once(int timeval_ms, int *n = NULL);

	void commit_spin_once() { _sbuf.pop_front(_rn); }

	SBuffer &buffer() { return _sbuf; }
};

bool CommRecv::setup(int sockfd)
{
	if (sockfd <= 0) return false;

	_sbuf.clear();
	_sockfd = sockfd;

	FD_ZERO(&_masterfs);
	// fake
	if (sockfd != 6188) {
		FD_SET(sockfd, &_masterfs);
	}
	_rc = CommRC::OK;
	return true;
}

size_t _recv_fake_svr_pack_data(char *buf)
{
	static long long cnt = 0;
	std::this_thread::sleep_for(std::chrono::milliseconds(100));

	float angle[6] = { 0.0f, 0.0f, 90.0f, 0.0f, 90.0f, 0.0f };
	float pose[6] = { 420.0f, -120.0f, 360.0f, 180.0f, 0.0f, 90.0f };
	FakeTmSvrPacket svr_pack;
	FakeTmSvrPacket::build_content(svr_pack.content, angle, pose);
	TmSvrData::build_TmSvrData(svr_pack.data, "0", TmSvrData::Mode::BINARY,
		svr_pack.content.data(), svr_pack.content.size(), TmSvrData::SrcType::Shallow);
	TmSvrData::build_bytes(svr_pack.packet.data, svr_pack.data);
	svr_pack.packet.setup_header(TmPacket::Header::TMSVR);
	std::vector<char> pack_byte;
	TmPacket::build_bytes(pack_byte, svr_pack.packet);
	size_t n = pack_byte.size();
	for (size_t i = 0; i < n; ++i) {
		buf[i] = pack_byte[i];
	}
	if (cnt % 10 == 1) {
		for (size_t j = 1; j < 7; ++j) {
			for (size_t i = 0; i < n; ++i) {
				buf[j * n + i] = pack_byte[i];
			}
		}
		n *= 7;
	}
	++cnt;
	return n;
}

CommRC CommRecv::spin_once(int timeval_ms, int *n)
{
	CommRC rc = CommRC::OK;
	int nb = 0;
	int rv = 0;
	//int sp = 0;
	timeval tv;

	// fake
	if (_sockfd == 6188) {
		nb = _recv_fake_svr_pack_data(_recv_buf);
		_sbuf.append(_recv_buf, nb);

		if (n) *n = nb;
		_rn = nb;
		_rc = rc;
		return rc;
	}

	if (timeval_ms < 8) timeval_ms = 8;

	tv.tv_sec = (timeval_ms / 1000);
	tv.tv_usec = (timeval_ms % 1000) * 1000;

	_readfs = _masterfs; // re-init

	rv = select(_sockfd + 1, &_readfs, NULL, NULL, &tv);

	if (n) *n = 0;
	
	if (rv < 0) {
		rc = CommRC::ERR;
	}
	else if (rv == 0) {
		rc = CommRC::TIMEOUT;
	}
	else if (FD_ISSET(_sockfd, &_readfs)) {
		nb = recv(_sockfd, _recv_buf, _recv_buf_len, 0);

		if (nb < 0) {
			// error
			rc = CommRC::ERR;
		}
		else if (nb == 0) {
			// sever is closed
			rc = CommRC::NOTCONNECT;
		}
		else {
			// recv n bytes
			_sbuf.append(_recv_buf, nb);

			if (n) *n = nb;
		}
	}
	else {
		rc = CommRC::NOTREADY;
	}
	_rn = nb;
	_rc = rc;
	return rc;
}

//
// Communication
//

Communication::Communication(const char *ip, unsigned short port, int recv_buf_len)
	: _recv(nullptr)
	, _ip(NULL)
	, _port(port)
	, _recv_buf_len(recv_buf_len)
	, _sockfd(-1)
	, _optflag(1)
	, _recv_rc(CommRC::OK)
	, _recv_ready(false)
{
	tmr_INFO_STREAM("tmr::Communication::Communication");

	_recv = new CommRecv(recv_buf_len);

	size_t len = strlen(ip);

	_ip = new char[len + 1];
	memcpy(_ip, ip, len);
	_ip[len] = '\0';

#ifdef _WIN32
	// Initialize Winsock
	WSADATA wsaData;
	int iResult = WSAStartup(MAKEWORD(2, 2), &wsaData);
	if (iResult != 0) {
		//
	}
#endif
}

Communication::~Communication()
{
	tmr_INFO_STREAM("tmr::Communication::~Communication");

	delete _ip;
	delete _recv;

#ifdef _WIN32
	// cleanup
	WSACleanup();
#endif
}

int Communication::connect_with_timeout(int sockfd, const char *ip, unsigned short port, int timeout_ms)
{
	int rv = 0;
	int flags = 0;
	int err = 0;
	int err_len = 0;
	sockaddr_in addr;
	timeval tv;
	fd_set wset;

	tmr_INFO_STREAM("TM_COM: ip:=" << ip);

	addr.sin_family = AF_INET;
	addr.sin_port = htons(port);
	inet_pton(AF_INET, ip, &(addr.sin_addr));

	tv.tv_sec = (timeout_ms / 1000);
	tv.tv_usec = (timeout_ms % 1000) * 1000;

	FD_ZERO(&wset);
	FD_SET(sockfd, &wset);

#ifndef _WIN32
	//Get Flag of Fcntl
	if ((flags = fcntl(sockfd, F_GETFL, 0)) < 0 ) {
		tmr_WARN_STREAM("TM_COM: The flag of fcntl is not ok");
		return -1;
	}
#endif

	rv = connect(sockfd, (sockaddr *)&addr, 16);
	tmr_INFO_STREAM("TM_COM: rv:=" << rv);

	if (rv < 0) {
		if (errno != EINPROGRESS) return -1;
	}
	if (rv == 0) {
		tmr_INFO_STREAM("TM_COM: Connection is ok");
		return rv;
	}
	else {
		//Wait for Connect OK by checking Write buffer
		if ((rv = select(sockfd + 1, NULL, &wset, NULL, &tv)) < 0) {
			return rv;
		}
		if (rv == 0) {
			tmr_WARN_STREAM("TM_COM: Connection timeout");
			//errno = ETIMEDOUT;
			return -1;
		}
		if (FD_ISSET(sockfd, &wset)) {
#ifdef _WIN32
			if (getsockopt(sockfd, SOL_SOCKET, SO_ERROR, (char*)&err, &err_len) < 0) {
#else
			if (getsockopt(sockfd, SOL_SOCKET, SO_ERROR, &err, (socklen_t *)&err_len) < 0) {
#endif
				tmr_ERROR_STREAM("TM_COM: Get socketopt SO_ERROR FAIL");
				errno = err;
				return -1;
			}
		}
		else {
			tmr_ERROR_STREAM("TM_COM: Connection is not ready");
			return -1;
		}
		if (err != 0) {
			errno = err;
			tmr_ERROR_STREAM("TM_COM: Connection error");
			return -1;
		}
	}
	return rv;
}

bool Communication::Connect(int timeout_ms)
{
	if (_sockfd > 0) return true;

	if (timeout_ms < 0) timeout_ms = 0;

#ifdef _WIN32
	addrinfo hints;
	ZeroMemory(&hints, sizeof(hints));
	hints.ai_family = AF_INET;
	hints.ai_socktype = SOCK_STREAM;
	hints.ai_protocol = IPPROTO_HOPOPTS;

	_sockfd = socket(hints.ai_family, hints.ai_socktype, hints.ai_protocol);
#else
	_sockfd = socket(AF_INET, SOCK_STREAM, 0);
#endif
	if (_sockfd < 0) {
		tmr_ERROR_STREAM("TM_COM: Error socket");
		return false;
	}

	if (setsockopt(_sockfd, IPPROTO_TCP, TCP_NODELAY, (char *)&_optflag, sizeof(_optflag)) < 0) {
		tmr_WARN_STREAM("TM_COM: setsockopt TCP_NODELAY failed");
	}
#ifndef _WIN32
	if (setsockopt(_sockfd, IPPROTO_TCP, TCP_QUICKACK, (char *)&_optflag, sizeof(_optflag)) < 0) {
		tmr_WARN_STREAM("TM_COM: setsockopt TCP_QUICKACK failed");
	}
#endif
	if (setsockopt(_sockfd, SOL_SOCKET, SO_REUSEADDR, (char *)&_optflag, sizeof(_optflag)) < 0) {
		tmr_WARN_STREAM("TM_COM: setsockopt SO_REUSEADDR failed");
	}
#ifdef _WIN32
	DWORD tv = timeout_ms;
	if (setsockopt(_sockfd, SOL_SOCKET, SO_SNDTIMEO, (char *)&tv, sizeof(tv)) < 0) {
		tmr_WARN_STREAM("TM_COM: setsockopt SO_SNDTIMEO failed");
	}
#else
	timeval tv;
	tv.tv_sec = (timeout_ms / 1000);
	tv.tv_usec = (timeout_ms % 1000) * 1000;
	// already timeout by select(...)
	//if (setsockopt(_sockfd, SOL_SOCKET, SO_RCVTIMEO, (char *)&tv, sizeof(tv)) < 0) {
	//	tmr_WARN_STREAM("TM_COM: setsockopt SO_RCVTIMEO failed");
	//}
	if (setsockopt(_sockfd, SOL_SOCKET, SO_SNDTIMEO, (char *)&tv, sizeof(tv)) < 0) {
		tmr_WARN_STREAM("TM_COM: setsockopt SO_SNDTIMEO failed");
	}
#endif

	if (connect_with_timeout(_sockfd, _ip, _port, timeout_ms) == 0) {
		tmr_INFO_STREAM("TM_COM: O_NONBLOCK connection is ok");
	}
	else {
		tmr_INFO_STREAM("TM_COM: O_NONBLOCK connection is fail");
		_sockfd = -1;
	}
	if (_sockfd > 0) {
		tmr_INFO_STREAM("TM_COM: TM robot is connected. sockfd:=" << _sockfd);
		//_is_connected = true;
		return true;
	}
	else {
		return false;
	}
}

void Communication::Close()
{
	// reset
	_recv_rc = CommRC::OK;
	_recv_ready = false;

	if (_sockfd <= 0) return;
#ifdef _WIN32
	closesocket((SOCKET)_sockfd);
#else
	close(_sockfd);
#endif
	_sockfd = -1;
}

CommRC Communication::send_bytes(const char *bytes, int len, int *n)
{
	CommRC rc = CommRC::OK;

	if (n) *n = 0;
	
	if (len <= 0) return CommRC::OK;
	if (_sockfd < 0) return CommRC::NOTREADY;

	int nb = send(_sockfd, bytes, len, 0);

	if (nb < 0) {
		rc = CommRC::ERR;
	}
	else if (nb < len) {
		rc = CommRC::NOTSENDALL;

		if (n) *n = nb;
	}
	return rc;
}

CommRC Communication::send_bytes_all(const char *bytes, int len, int *n)
{
	CommRC rc = CommRC::OK;

	if (n) *n = 0;

	if (len <= 0) return CommRC::OK;
	if (_sockfd < 0) return CommRC::NOTREADY;

	int ntotal = 0;
	int nb = 0;
	int nleft = len;

	while (ntotal < len) {
		nb = send(_sockfd, bytes, nleft, 0);
		if (nb < 0) {
			rc = CommRC::ERR;
			break;
		}
		ntotal += nb;
		nleft -= nb;
	}
	if (n) *n = ntotal;
	return rc;
}

CommRC Communication::send_packet(TmPacket &packet, int *n)
{
	std::vector<char> bytes;
	TmPacket::build_bytes(bytes, packet);
	tmr_INFO_STREAM(TmPacket::string_from_bytes(bytes));
	return send_bytes(bytes.data(), bytes.size(), n);
}
CommRC Communication::send_packet_all(TmPacket &packet, int *n)
{
	std::vector<char> bytes;
	TmPacket::build_bytes(bytes, packet);
	tmr_INFO_STREAM(TmPacket::string_from_bytes(bytes));
	return send_bytes_all(bytes.data(), bytes.size(), n);
}
CommRC Communication::send_packet_(TmPacket &packet, int *n)
{
	std::vector<char> bytes;
	TmPacket::build_bytes(bytes, packet);
	tmr_INFO_STREAM(TmPacket::string_from_bytes(bytes));
	if (bytes.size() > 0x1000)
		return send_bytes_all(bytes.data(), bytes.size(), n);
	else
		return send_bytes(bytes.data(), bytes.size(), n);
}

CommRC Communication::send_packet_silent(TmPacket &packet, int *n)
{
	std::vector<char> bytes;
	TmPacket::build_bytes(bytes, packet);
	return send_bytes(bytes.data(), bytes.size(), n);
}
CommRC Communication::send_packet_silent_all(TmPacket &packet, int *n)
{
	std::vector<char> bytes;
	TmPacket::build_bytes(bytes, packet);
	return send_bytes_all(bytes.data(), bytes.size(), n);
}
CommRC Communication::send_packet_silent_(TmPacket &packet, int *n)
{
	std::vector<char> bytes;
	TmPacket::build_bytes(bytes, packet);
	if (bytes.size() > 0x1000)
		return send_bytes_all(bytes.data(), bytes.size(), n);
	else
		return send_bytes(bytes.data(), bytes.size(), n);
}

bool Communication::recv_init()
{
	_recv_ready = _recv->setup(_sockfd);
	return _recv_ready;
}

CommRC Communication::recv_spin_once(int timeval_ms, int *n)
{
	CommRC rc = CommRC::OK;

	if (n) *n = 0;

	//if (_sockfd <= 0) return CommRC::NOTCONNECT;

	// first init.
	/*if (!_recv_ready) {
		if (_recv->setup(_sockfd))
			_recv_ready = true;
		else
			return CommRC::NOTREADY;
	}*/

	// spin once
	int nb = 0;
	rc = _recv->spin_once(timeval_ms, &nb);
	
	if (n) *n = nb;

	// error handling
	if (rc != CommRC::OK) {
		_recv_rc = rc;
		return rc;
	}

	// find packet
	int loop_cnt = 0;
	int pack_cnt = 0;
	int blen = 0;
	char *bdata = NULL;
	size_t size = 0;
	size_t len = 0;
	bool ncs = false;
	bool ok = false;

	while (loop_cnt < 10 || pack_cnt < 10) {

		blen = _recv->buffer().length();
		if (blen < 9) {
			break;
		}
		bdata = _recv->buffer().data();

		//tmr_DEBUG_STREAM("tmr::Communication::recv_spin_once: " << bdata << ", " << loop_cnt);

		++size;
		_packet_list.resize(size);

		len = TmPacket::build_packet_from_bytes(_packet_list.back(), bdata, blen);

		ncs = packet().is_checksum_failed();
		ok = packet().is_valid();

		if (ok || ncs) {
			_recv->buffer().pop_front(len);
		}
		if (ok) {
			++pack_cnt;
		}
		else {
			if (size > 1) {
				_packet_list.resize(size - 1);
			}
			//if (pack_cnt != 0) break;
			break;
		}
		++loop_cnt;
	}
	if (pack_cnt == 0) {
		rc = CommRC::NOVALIDPACK;
	}
	_recv_rc = rc;
	return rc;
}

}